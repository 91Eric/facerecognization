...
int main(int argc, char** argv)
{
    ...
    for (int32_t i = 0; i < num_face; i++)
    {
        ...
        for(auto point : points_known)
        {
            cv::circle(img_origin_known, cv::Point(static_cast<int>(point.x), static_cast<int>(point.y)), 2, cv::Scalar(0, 0, 255), 2);
        }

         //bbox of unknown people
        face_rect.x = faces_unknown[i].bbox.x;
        face_rect.y = faces_unknown[i].bbox.y;
        face_rect.width = faces_unknown[i].bbox.width;
        face_rect.height = faces_unknown[i].bbox.height;

        cv::rectangle(img_origin_unknown, face_rect, CV_RGB(0, 0, 255), 4, 8, 0);
        ...
        for(auto point : points_unknown)
        {
            cv::circle(img_origin_unknown, cv::Point(static_cast<int>(point.x), static_cast<int>(point.y)), 2, cv::Scalar(0, 0, 255), 2);
        }
    }
